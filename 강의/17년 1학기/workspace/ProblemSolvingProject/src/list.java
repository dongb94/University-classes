...
public class list {
   ...
   public static void main(String[] args) {
      try {
         ...
         Scanner scan = new Scanner(new File("packing_input.txt"));
         int T = scan.nextInt();      //테스트 케이스 수
         int count = 1;
         while (T-- > 0) {

            int number = scan.nextInt();   	    //N의 크기
            int width = scan.nextInt();         //L의 크기
            int total = 0;
            int min = 0;
            int len = 0;
            int array[] = new int[number * 2];
            for (int i = 0; i < number; i++) {
               array[i * 2] = scan.nextInt();            //가로
               array[i * 2 + 1] = scan.nextInt();         //세로
            ...
            for (int i = 0; i < 1000; i++) { // 포문으로 회전 200번을 통해 제일
               // optimize한 결과를 출력
               while (left <= right) {
