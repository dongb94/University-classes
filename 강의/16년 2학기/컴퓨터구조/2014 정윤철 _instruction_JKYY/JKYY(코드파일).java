...
public class JKYY {
	public static void main(String[] args) {
		...
		randomArray(MEMORY); // 배열에 21개의 랜덤값을 채워 넣는 메소드
		

		System.out.printf("%8s%11s%16s%22s\n","location","16 code","symbol","binary code");

		for(int i = 0; i < 21; i ++){
			
			System.out.printf("%8s", Integer.toString(i + 100).substring(1));  
			// 인덱스 출력
			
			System.out.printf("%11s", Integer.toHexString(MEMORY[i] + 0x10000).substring(1)); 
			// 16진수 코드 출력
			
			System.out.printf("%16s", symbolCheck(MEMORY[i])); 
			// instruction 심볼 + 가리키는 주소값 String 을 리턴하는 메소드 출력
			
			System.out.printf("%22s\n", Integer.toBinaryString(MEMORY[i] + 0x10000).substring(1)); 
			// 2진수 코드 출력
		}
